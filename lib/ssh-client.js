/* jshint node: true */
'use strict';

var CoreObject = require('core-object');
var Promise    = require('rsvp').Promise;
var SSH2Client = require('ssh2').Client;
var fs         = require('fs');
var untildify  = require('untildify');
var path       = require('path');


module.exports = CoreObject.extend({

  init: function(options) {
    if (options.agent) {
      delete options["privateKeyPath"];
    }

    if (options.privateKeyPath) {
      options.privateKey = fs.readFileSync(untildify(options.privateKeyPath));
    }

    this.options = options;
    this.client  = new SSH2Client();
  },


  connect: function() {
    var client  = this.client;
    var options = this.options;

    client.connect(options);

    return new Promise(function(resolve, reject) {
      client.on('error', reject);
      client.on('ready', resolve);
    });
  },

  disconnect: function() {
    var client = this.client;

    client.end();

    return new Promise(function(resolve, reject) {
      client.on('error', reject);
      client.on('end', resolve);
    });
  },

  upload: function(path, data) {
    var client = this.client;

    return new Promise(function (resolve, reject) {
      client.sftp(function(error, sftp) {
        if (error) {
          reject(error);
        }

        var stream = sftp.createWriteStream(path);

        stream.on('error', reject);
        stream.on('finish', resolve);
        stream.on('close', function() {
          sftp.end();
        });
        stream.write(data);
        stream.end();
      });
    });
  },


  readFile: function(path) {
    var client = this.client;

    return new Promise(function(resolve, reject) {
      client.sftp(function(error, sftp) {
        if (error) {
          reject(error);
        }

        sftp.readFile(path, {}, function (error, data) {
          sftp.end();

          if (error) {
            reject(error);
          } else {
            resolve(data);
          }
        });
      });
    });
  },


  exec: function(command) {
    var client = this.client;

    return new Promise(function(resolve, reject) {
      client.exec(command, function(error/*, stream*/) {
        if (error) {
          reject(error);
        }
        resolve();
      });
    });
  },


  putFile: function(src, dest) {
    var _this  = this;
    var client = this.client;

    return new Promise(function(resolve, reject) {
      _this.ensureDirectory(dest).then(
        function() {
          client.sftp(function (err, sftp) {
            if (err) {
              reject(err);
            }

            _this.fastPut(sftp, src, dest).then(resolve, reject);
          });
        },
        reject
      );
    });
  },

  putFiles: function(files, distDir, destination) {
    var _this = this;
    var client = this.client;

    return new Promise(function(resolve, reject) {
      client.sftp(function (err, sftp) {
        if (err) {
          reject(err);
        }

        var promises = [];
        files.forEach(function(file) {
          var src = path.join(distDir, file);
          var dest = path.posix.join(destination, file);

          var promise = _this.ensureDirectory(dest).then(function(){
            return _this.fastPut(sftp, src, dest)
          }, reject);
          promises.push(promise);
        });

        Promise.all(promises).then(function() {
          sftp.end();
          resolve();
        }, reject);
      });
    });
  },

  fastPut: function(sftp, src, dest) {
    return new Promise(function(resolve, reject) {
      sftp.fastPut(src, dest, {}, function(err) {
        if (err) {
          reject(err);
        }
        resolve();
      });
    });
  },

  ensureDirectory: function(path) {
    var parts = path.split('/');
    parts.pop();
    var dir = parts.join('/');

    return this.exec('mkdir -p ' + dir);
  }
});
